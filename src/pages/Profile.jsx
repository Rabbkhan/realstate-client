...
import { updateUserStart, updateUserFailure, updateUserSuccess, deleteUserFailure, deleteUserSuccess, signoutUserStart, signInFailure, signInSuccess } from '../redux/user/userSlice'
...
const Profile = () => {
  ...
  const { currentUser, isUpdateuser, isSigningOut } = useSelector((state) => state.user);
...
toast.success("Listing deleted successfully!")
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gray-100 py-12">
      <form onSubmit={handleSubmit} className="w-full max-w-2xl bg-white rounded-lg shadow-lg p-8">

        {/* Header and Avatar */}
        <div className="flex items-center justify-between border-b pb-4 mb-6">
          <div className="flex items-center gap-4">
            <div className="relative">
              <input onChange={(e) => setFile(e.target.files[0])} type='file' ref={fileRef} className='hidden' />
              <img
              ...
              <button type="button" onClick={() => fileRef.current.click()} className="absolute bottom-0 right-0 bg-blue-500 text-white p-1 rounded-full hover:bg-blue-600">
        ...
        <div>
          ...
          {fileUploadError ? (toast.error("Error Image Upload (image must be less then 2mb)")) :
        ...
        <div className="mt-8 flex flex-row space-x-4 gap-2 flex-wrap justify-center">
          <button
            ...
            type="submit"
            className={`flex items-center justify-center px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg shadow-lg transition duration-200 ease-in-out hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 ${isUpdateuser ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
          ...
          <button
            ...
            className={`flex items-center justify-center px-4 py-2 bg-gray-500 text-white font-semibold rounded-lg shadow-lg transition duration-200 ease-in-out hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-400 ${isSigningOut ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
...
{userListings && userListings.length > 0 && userListings.map((listing) => (
  <div key={listing._id} className="flex items-center border border-gray-300 rounded-lg shadow-lg p-4 m-4 max-w-4xl w-full lg:w-3/4 cursor-pointer">
    <Link to={`/listing/${listing._id}`} className="flex-shrink-0 w-24 h-24">
      <img src={listing.imageUrl} alt={listing.title} className="w-full h-full object-cover rounded-md" />
    </Link>
    
    <div className="flex-grow ml-4">
      <Link to={`/listing/${listing._id}`}>
        <div className="text-lg font-semibold text-gray-800 line-clamp-2">{listing.description}</div>
      </Link>
    ...
    <div className="flex space-x-2 ml-auto">
     ...
     <button
        className="px-4 py-1.5 bg-blue-500 text-white rounded-md hover:bg-blue-600 transition-colors duration-200"
      >
