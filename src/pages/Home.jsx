  ...
  <div className="max-w-6xl mx-auto px-6 sm:px-12 text-center">
    <div className="text-black">
      ...
      <p className="text-lg sm:text-xl font-light text-black max-w-2xl mx-auto mb-8">
        Find your perfect home, whether it’s an apartment, villa, or commercial space. 
        Explore verified listings with transparent pricing and hassle-free navigation.
      </p>
    </div>
    ...
    <div className="mt-12 text-black">
      <p className="text-md sm:text-lg text-black">
        Begin your journey now. <span className="text-blue-400 font-medium">Explore exclusive listings</span> in prime locations today.
      </p>
    </div>
  </div>
...
<Swiper navigation>
  {
    offerListings && offerListings.length > 0 && offerListings.map((listing)=>(
      <SwiperSlide>
        <div  style={{background: `url(${listing.imageUrl[0]}) center no-repeat`, backgroundSize:"cover"}} className='h-[600px]' key={listing._id}>
...
<div className="container mx-auto p-4">
  ...
  <div className="flex justify-between items-center mb-6">
    <h2 className="text-3xl font-semibold text-gray-800">Offer Listings</h2>
    {/* Show More Link */}
    <Link
    ...
    offerListings.map((offerlisting, index) => (
      ...
      >
        ...
        <div className="p-4">
          ...
          <p className="text-gray-600 text-sm mt-1 line-clamp-3 ">{offerlisting.description}</p>
        ...
        <div className="flex gap-3 items-center p-4 border-t">
          <p className="flex items-center gap-2 font-semibold">
            {offerlisting.bathrooms > 1 && <FaBath className="text-blue-600" />}
            {offerlisting.bathrooms > 1
              ? ` ${offerlisting.bathrooms} baths `
              : ` ${offerlisting.bathrooms} bath `}
          </p>
          <p className="flex items-center gap-2 font-semibold">
            {offerlisting.bedrooms > 1 && <FaBed className="text-blue-600" />}
...
<div className="container mx-auto p-4">
  {/* Rent Heading */}
  <div className="flex justify-between items-center mb-6">
    <h2 className="text-3xl font-semibold text-gray-800">Rent Listings</h2>
    {/* Show More Link */}
    <Link
      to="/search?type=rent"
      className="text-blue-600 font-semibold hover:text-blue-800 transition-colors duration-200"
    ...
    rentListings.map((rentlisting, index) => (
      ...
      >
        ...
        <div className="p-4">
          ...
          <p className="text-gray-600 text-sm mt-1 line-clamp-3 ">{rentlisting.description}</p>
          <p className="mt-4 text-lg font-bold text-blue-600">
            ${rentlisting.offer
        ...
        <div className="flex gap-3 items-center p-4 border-t">
          <p className="flex items-center gap-2 font-semibold">
            {rentlisting.bathrooms > 1 && <FaBath className="text-blue-600" />}
            {rentlisting.bathrooms > 1
              ? ` ${rentlisting.bathrooms} baths `
              : ` ${rentlisting.bathrooms} bath `}
          </p>
          <p className="flex items-center gap-2 font-semibold">
            {rentlisting.bedrooms > 1 && <FaBed className="text-blue-600" />}
...
<div className="flex justify-between items-center mb-6">
    <h2 className="text-3xl font-semibold text-gray-800">Sale Listings</h2>
    {/* Show More Link */}
    <Link
      to="/search?type=sale"
      className="text-blue-600 font-semibold hover:text-blue-800 transition-colors duration-200"
      ...
      saleListings.map((saleListing, index) => (
        ...
        >
          ...
          <div className="p-4">
            ...
            <p className="text-gray-600 text-sm mt-1 line-clamp-3 ">{saleListing.description}</p>
            <p className="mt-4 text-lg font-bold text-blue-600">
              ${saleListing.offer
          ...
          <div className="flex gap-3 items-center p-4 border-t">
            <p className="flex items-center gap-2 font-semibold">
              {saleListing.bathrooms > 1 && <FaBath className="text-blue-600" />}
              {saleListing.bathrooms > 1
                ? ` ${saleListing.bathrooms} baths `
                : ` ${saleListing.bathrooms} bath `}
            </p>
            <p className="flex items-center gap-2 font-semibold">
              {saleListing.bedrooms > 1 && <FaBed className="text-blue-600" />}
